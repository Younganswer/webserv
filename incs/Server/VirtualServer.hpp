...
class VirtualServer {
	private:
		...
		const ft::Trie<Location>		_locations;
	...
	public:
		...
		const ft::Trie<Location>		&getLocations(void) const;
	...
	private:
		...
		static ft::Trie<Location>		_parseLocations(const Config::map &config_map);
	...
	//Custom will be added
		...
		static bool						_locationsIsValid(const ft::Trie<Location> &locations);
	//Custom will be added
